...
def test_search_node_state_type():
    ...
    node = SearchNode(
        "searcher", config, "Find {{ query }} in {{ context }}", state_type=StateForTest
    )
...
def test_search_node_tools_injection():
    ...
    from google.adk.tools.google_search_tool import (
        GoogleSearchTool as AdkGoogleSearchTool,
    )
...
async def test_search_node_execute():
    ...
    config = ADKConfig(model_name="gemini-3-flash-preview")
    node = SearchNode("searcher", config, "Find {{ query }}", mock_responder=mock_resp)
