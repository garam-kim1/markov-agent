...
from markov_agent import ADKConfig, Agent, model_config
...
def test_agent_initialization():
    agent = Agent(
        name="test_agent", model="gemini-1.5-flash", system_prompt="You are a tester."
    )
...
async def test_agent_run_mock():
    ...
    agent = Agent(name="test_agent", mock_responder=mock_responder)
