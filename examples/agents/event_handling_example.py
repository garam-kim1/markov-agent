        ...
        if hasattr(model_request, 'contents'):
            for content in model_request.contents:
                for part in content.parts:
                    if hasattr(part, 'text') and part.text and "secret" in part.text.lower():
                        has_secret = True
                        break
        
        if has_secret:
            print("[Callback] BLOCKING request due to security policy (contains 'secret').")
            return None 

        return None

# 2. The Event Loop: Consuming Events
async def run_agent_event_loop(controller: ADKController, user_input: str, session_id: Optional[str] = None, user_id: str = "system"):
    print(f"\n--- Running Event Loop for: '{user_input}' ---")
    
    # Using the new run_async method in ADKController
    async for event in controller.run_async(prompt=user_input, session_id=session_id, user_id=user_id):
...
async def main():
    ...
    async for _ in controller.run_async("Who is the president of the USA?", session_id=session_id, user_id=user_id):
        pass
    ...
    for event in events:
        if event.is_final_response():
            print(f" - Saved Response: {event.content.parts[0].text[:50]}...")
