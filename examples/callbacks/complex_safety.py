    ...
    def __call__(self, context, model_request):
        ...
        for content in model_request.contents:
            for part in content.parts:
                if part.text:
                    scrubbed_text = re.sub(self.EMAIL_REGEX, "[REDACTED_EMAIL]", part.text)
                    if scrubbed_text != part.text:
                        print(f"[Security] Redacting PII from prompt...")
    ...
    def __call__(self, context, model_response):
        ...
        for term in self.FORBIDDEN_TERMS:
            if term in text_content:
                ...
                raise CallbackException(f"Policy Violation: Generated content contains forbidden term '{term}'")
...
async def main():
    ...
    try:
        ...
        if "API_KEY" in str(e) or "403" in str(e) or "401" in str(e) or "credentials" in str(e):
